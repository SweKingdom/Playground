...
public static class Excericee0201
{
    ...
    {      
        ...
        foreach (var name in rockGroups)
        ...
        foreach (var group in oldestGroups)
            Console.WriteLine($"{group.EstablishedYear}: {group.Name}");
        ...
        var copiesByGenre = musicgroups
            ...
            .Select(g=>
                new {Genre=g
                    ...
                    .Sum(album=>album.CopiesSold)});
        foreach (var item in copiesByGenre)
            Console.WriteLine($"{item.Genre}: {item.TotalCopies} copies sold");
        ...
        foreach (var group in largeGroups)
            Console.WriteLine($"{group.Name} ({group.Artists.Count} artists)");
        ...
        var avgTracks = musicgroups
            .Select(g => new
            {
                GroupName = g.Name,
                AvgTracks = g.Albums
                    .SelectMany(a => new[] { a.Tracks.Count })   // flatten album track counts
                    .Average()
            });
        foreach (var group in avgTracks)
            Console.WriteLine($"{group.GroupName}: {group.AvgTracks} tracks per album on average");
        ...
        var successfulJazzArtists = musicgroups
            ...
            .Where(g => g.Albums.Any(a => a.CopiesSold > 500000))
            .SelectMany(g => g.Artists)
            .Distinct();
        foreach (var artist in successfulJazzArtists)
        ...
        var mostProlificByGenre = musicgroups
            .GroupBy(g => g.Genre)
            .Select(g => g
                .OrderByDescending(group => group.Albums.Count)
                .Select(group => new
                {
                    Genre = g.Key,
                    GroupName = group.Name,
                    AlbumCount = group.Albums.Count
                })
                .First());
        foreach (var item in mostProlificByGenre)
            Console.WriteLine($"{item.Genre}: {item.GroupName} ({item.AlbumCount} albums)");
        ...
        var longestTracks = musicgroups
            .SelectMany(g => g.Albums, (group, album) => new { group, album })
            .SelectMany(x=>x.album.Tracks,
                (x, track) => new
                {
            ...
            .Take(10);
        foreach (var track in longestTracks)
            Console.WriteLine(
                $"{track.Duration:mm\\:ss} - {track.TrackName} " +
                $"({track.GroupName} / {track.AlbumName})");
        ...
        var metalByDecade = musicgroups
            ...
            .Select(g => new
            {
                Decade = $"{g.Key}s",
                AlbumCount = g.Count(),
                Percentage = g.Count() * 100.0 /
                             musicgroups
                                 .Where(mg => mg.Genre == MusicGenre.Metal)
                                 .SelectMany(mg => mg.Albums)
                                 .Count()
            })
            .OrderBy(x=> x.Decade);
        foreach (var item in metalByDecade)
            Console.WriteLine(
                $"{item.Decade}: {item.Percentage:F1}% ({item.AlbumCount} albums)");
